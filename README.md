...
**实测：全新 clone 不配任何配置文件也能跑通 `pytest`（808 项）与 `--resolve` 纯规则链路。**
...
| 脚本 | 作用 |
|---|---|
| **`start-web.cmd`** | 起本地服务，约 4 秒后自动打开浏览器到 <http://127.0.0.1:8080>。设 `TA_NO_BROWSER=1` 可跳过自动开浏览器 |
...
* 本地界面：<http://127.0.0.1:8080>
* 接口文档：<http://127.0.0.1:8080/docs>
* 自检：<http://127.0.0.1:8080/healthz>
...
```powershell
cd core
python -m pytest tests -q    # 808 passed，不调用模型、不需要 API Key
```
...
> 本表只列核心 7 个文件；`core/tests/` 现共 **40 个测试文件 / 808 项**，完整清单跑 `pytest --collect-only -q`（在 `core/` 目录下）。
...
python -m pytest core\tests -q       # 808 项
```
