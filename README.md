...
**实测：全新 clone 不配任何配置文件也能跑通 `pytest`（786 项）与 `--resolve` 纯规则链路。**
...
```powershell
cd core
python -m pytest tests -q    # 786 passed，不调用模型、不需要 API Key
```
...
> 本表只列核心 7 个文件；`core/tests/` 现共 **38 个测试文件 / 786 项**，完整清单跑 `pytest --collect-only -q`（在 `core/` 目录下）。
...
python -m pytest core\tests -q       # 786 项
```
