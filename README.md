...
**实测：全新 clone 不配任何配置文件也能跑通 `pytest`（252 项）与 `--resolve` 纯规则链路。**
...
```powershell
cd core
python -m pytest -q          # 252 passed，约 0.7 秒，不调用模型、不需要 API Key
```
