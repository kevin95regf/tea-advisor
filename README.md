...
**实测：全新 clone 不配任何配置文件也能跑通 `pytest`（812 项）与 `--resolve` 纯规则链路。**
...
| 脚本 | 作用 |
|---|---|
| **`start-web.cmd`** | 起本地服务，约 4 秒后自动打开浏览器到 <http://127.0.0.1:8120>。设 `TA_NO_BROWSER=1` 可跳过自动开浏览器 |
...
```powershell
cd core
python -m uvicorn app.main:app --reload --port 8120
```

* 本地界面：<http://127.0.0.1:8120>
* 接口文档：<http://127.0.0.1:8120/docs>
* 自检：<http://127.0.0.1:8120/healthz>
...
```powershell
cd core
python -m pytest tests -q    # 812 passed，不调用模型、不需要 API Key
```
...
> 本表只列核心 7 个文件；`core/tests/` 现共 **41 个测试文件 / 812 项**，完整清单跑 `pytest --collect-only -q`（在 `core/` 目录下）。
...
python -m pytest core\tests -q       # 812 项
```
