...
ENTRY_FIXES: dict[str, dict] = {
    ...
    # ---------- D29：单字关键词的跨条目误命中 ----------
    # 「鸡蛋」的 keywords 末项是裸字「蛋」。匹配是**子串**级 ⇒ 输入「蛋糕」会同时
    # 命中「蛋糕」与「鸡蛋」（实测 `match_foods('蛋糕') -> ['蛋糕', '鸡蛋']`）。
    # ⚠️ 判据不是「长度」而是「**是否作为子串出现在别的条目里**」：全表 15 个单字
    # keywords 里只有 6 个踩中（鱼/姜/梨/糖/盐/蛋），而「粥/虾/蟹/葱/蒜/藕/柑/醋/馍」
    # 都是独立食物名、命中是对的 ⇒ 一刀切禁单字会误伤这 9 个。
    # 本条只处理「蛋」（它带来的误命中跨了物种：蛋糕不是鸡蛋）。
    "jidan": {
        "keywords": ["鸡蛋", "水煮蛋", "煎蛋", "荷包蛋"],
        "review_note": (
            "D29：移除裸字「蛋」——它是别的条目词（蛋糕/卤蛋/茶叶蛋）的子串，"
            "会让「蛋糕」同时命中鸡蛋。其余单字（鱼/姜/梨/糖/盐）同类问题已登记，"
            "由 tests/test_single_char_keywords.py 的白名单守着，新增即变红。"
        ),
    },
}
