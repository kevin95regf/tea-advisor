...
| 费用 | 记在**调用方自己的账号**上。高风险分支与离线分支**一分钱不花**（不调用模型） |
...
### 2.2 模块职责（行数为实测 LF 行数）

| 文件 | 行 | 职责 |
|---|---|---|
| `core/app/main.py` | 205 | FastAPI 装配：路由注册、CORS（只放行本机来源）、lifespan 启动自检、`/healthz`、`/api/meta`、`/api/constitutions`、`/api/catalog/herbs`、`/api/disclaimer`、`/` 与 `/demo` 返回网页 |
| `core/app/config.py` | 129 | 环境变量集中读取（唯一入口）。**不读任何 API Key**；`.env` + `credentials.env` 两处加载 |
| `core/app/api/analyze.py` | 67 | `POST /api/analyze`。取 Bearer Key → `orchestrator.analyze()`；错误码分档 400/422/500 |
| `core/app/api/analyze_offline.py` | 215 | `POST /api/analyze-offline`。纯规则链路，不需要 Key，一次模型都不调 |
| `core/app/api/auth.py` | 47 | 只做一件事：把 `Authorization: Bearer <key>` 解析成裸 Key。不写日志、不入异常 |
| `core/app/api/questionnaire.py` | 126 | `POST /api/questionnaire`、`GET /api/questionnaire/questions`。依赖可选子包 |
| `core/app/api/chat.py` | 197 | `POST /api/chat` + `GET /api/chat/models` + `GET /api/chat/personas`。**兼容对话入口，不是茶饮主流程** |
| `core/app/api/medication.py` | 48 | `GET /api/medication-reference[/{constitution}]`。只读展示，不参与推荐链路 |
| `core/app/services/orchestrator.py` | 378 | **唯一编排入口** `analyze()`：三道前置闸门 → Agent1 → Agent2 → 输出护栏 → 组装；`_build_basis()` 是 `Basis` 的唯一构造点 |
| `core/app/services/matcher.py` | 595 | 规则兜底 `fallback_recommend()`、场景规则、体质默认搭配、冲泡常量、优先级链 L0–L4 |
| `core/app/services/food_lookup.py` | 663 | 三层判定入口 `resolve_food()`、匹配 `match_foods()`/`_find_entry()`、温度层准入、置信度常量、提示词参考表渲染 |
| `core/app/services/medication_reference.py` | 220 | 用药调理资料加载与结构校验（显式失败），与推荐链路硬隔离 |
| `core/app/agents/agent1_diet.py` | 195 | 饮食解析器：拼提示词（含参考表）、调运行时、`json_guard` 校验、`calibrate_parsed()` 校准 |
| `core/app/agents/agent2_recommend.py` | 257 | 推荐器：候选清单渲染、优先级渲染、未验证项隔离节、调运行时、`json_guard` 校验 |
| `core/app/agents/runtime.py` | 251 | 后端选择：`direct`（默认，支持逐请求 Key）/ `dsh`（子进程，Key 绑定）；`CredentialError` |
| `core/app/agents/direct_api.py` | 235 | 直连官方 API：thinking 参数映射、120 s 超时、状态码→错误类型映射 |
| `core/app/agents/multi_provider.py` | 255 | 多供应商（deepseek/qwen/mimo/hy4），**只被 `/api/chat` 使用** |
| `core/app/agents/json_guard.py` | 166 | 围栏剥离 → 对象提取 → 宽松解析 → schema 校验 → 失败重试 1 次 |
| `core/app/agents/prompts/agent1_system.md` | — | Agent1 系统提示词 |
| `core/app/agents/prompts/agent2_system.md` | — | Agent2 系统提示词（含"硬性边界"8 条） |
| `core/app/domain/safety.py` | 514 | **确定性安全护栏**：高风险、禁用表述、白名单、剂量、体质收敛、煎煮适配、依据链 |
| `core/app/domain/models.py` | 361 | 请求/响应/中间结构的**唯一真源** |
| `core/app/domain/enums.py` | 134 | 四气/五味/时段/烹饪/九型体质的枚举与中文标签 |
| `core/app/domain/nature_math.py` | 462 | 四性数值编码、夹取、两层修正、合成 `combine()`、温度前缀唯一入口、寒热错杂 |
| `core/app/domain/diet_signals.py` | 678 | 饮食信号识别与计分（冲击度/湿气度/寒热错杂）、餐单派生 `derive_meal_plan()` |
...
### 2.5 对外接口面（全部实测注册于 `main.py:110-115` 与 `main.py:120-205`）

| 方法 | 路径 | 需要 Key | 用途 |
|---|---|---|---|
| POST | `/api/analyze` | **是** | 主流程：口述 → 茶饮推荐 |
| POST | `/api/analyze-offline` | 否 | 离线规则推荐（不调模型） |
| POST | `/api/questionnaire` | 否 | 问卷计分 → 主导体质 + 兼夹屏蔽集 |
| GET | `/api/questionnaire/questions?sex=` | 否 | 取题目（默认 `male`） |
| POST | `/api/chat` | **是** | 兼容对话入口（多模型），**非茶饮主流程** |
| GET | `/api/chat/models` | 否 | 6 个硬编码模型选项 |
| GET | `/api/chat/personas` | 否 | 旧版兼容接口，**固定返回空数组** |
| GET | `/api/medication-reference` | 否 | 用药调理资料目录（只读） |
| GET | `/api/medication-reference/{constitution}` | 否 | 单个体质的资料，未收录 → 404 |
| GET | `/healthz` | 否 | 自检（含 `user_key_supported`，前端据此提示后端是否支持自带 Key） |
| GET | `/api/meta` | 否 | 展示用中文标签 + `conf_show_threshold` + 食性表规模 |
| GET | `/api/constitutions` | 否 | 可对外服务的体质（按 `ready_constitutions()` 过滤） |
| GET | `/api/catalog/herbs` | 否 | 饮片目录（44 味） |
| GET | `/api/disclaimer` | 否 | 免责声明文案（避免各壳硬编码） |
| GET | `/`、`/demo` | 否 | 同一个网页（`/demo` 为历史路径保留） |
...
| `TA_PROVIDER` / `TA_MODEL` | `deepseek-official` / `deepseek-v4-flash` | 模型选择 |
| `TA_REASONING_EFFORT` | `low` | 本模型支持 `max/high/low/off`；`medium`/`none`/`minimal` 会导致启动失败。默认 `low` 是因为 `off` 虽快约 3 倍但推荐常只剩 1 条 |
...
保留最近 20 条上下文；可选择 6 个模型；沿用用户自带 Key。**先跑高风险检测**——命中则直接返回固定就医话术并标 `safety_intercepted=true`，**此时不检查 Key、不调用模型**；无 Key 返回 400。模型输出经禁用表述扫描，不合规则替换为固定文案。凭据错误 → 400，运行错误 → 502。
...
> 全部结构的**唯一真源**是 `core/app/domain/models.py`。改字段名等于改对外契约，必须同步前端与提示词。

### 4.1 请求

| 对象 | 字段 | 说明 |
|---|---|---|
| `AnalyzeRequest` | `text`(1–500，去空白后不可为空)、`meal_time`、`constitution_override`、`exclude_herbs`、`session_id`、`avoid_constitutions` | `avoid_constitutions` 是**兼夹屏蔽集**：这些体质标为"不宜"的饮片一并排除，但**不改变收敛方向**（方向仍由 `constitution_override` 唯一决定） |
| `OfflineAnalyzeRequest` | `text`(≤500)、`constitution`、`constitution_override`、`avoid_constitutions`、`exclude_herbs` | **没有 Key 字段** |

### 4.2 响应

| 对象 | 字段 |
|---|---|
| `AnalyzeResponse` | `request_id`、`parsed`、`recommendations[]`、`basis`、`disclaimer`、`meta`、`user_message` |
| `Basis` | `constitution`、`constitution_label`、`rule_hits[]`、`guardrail_applied[]`、`references[]` |
| `EvidenceReference` | `id`、`title`、`publisher`、`url`、`supports[]`、`note`（来源自身的注意事项，**前端不得省略**） |
| `Meta` | `agent1_ms`、`agent2_ms`、`total_ms`、`model`、`degraded`、`degraded_reason`、`key_source`(`user`/`not_used`)、`backend` |
| `Disclaimer` | `version`、`text`、`is_medical_advice`(**恒为 false**) |
| `HealthResponse` | `status`、`herbs_loaded`、`data_files_ok`、`dsh_home`、`dsh_home_exists`、`model`、`disclaimer_version`、`backend`、`user_key_supported`（刻意**没有** `credentials_ok`——"服务端有没有 Key"不是有意义的状态） |

### 4.3 中间对象

| 对象 | 字段 | 说明 |
|---|---|---|
| `ParsedFood` | `name`、`amount_desc`、`nature`、`flavors[]`、`cooking`、`note`、`verification` | `note` 是**运行时字段**：温度前缀层会扫它 |
| `ParsedMeal` | `foods[]`、`meal_time`、`overall_nature`、`confidence`、`uncertain_items[]`、`summary`、`signals`、`plan` | `signals`/`plan` 为 `None` 表示未计算，既有关联方不传即保持旧行为 |
| `Verification` | `source`、`confidence`、`unverified`、`detail` | 三层架构的元数据出口 |
| `MealDimension` | `label`、`score`、`cap`、`action`、`action_label`、`signals[]`、`evidence_floor`、`basis` | 阈值与中文标签**全部来自数据文件**，代码不写死 |
| `MealConflict` | `conflict`、`heat_side[]`、`cold_side[]` | 值必须**原样**来自 `nature_math.detect_nature_conflict`，组装层不得另判一套 |
| `MealSignals` | `impact`、`dampness`、`conflict` | 三者都可能为 `None`（维度定义缺失时不臆造结论） |
| `MealPlan` | `first`、`second`、`deferred`(默认 **true**)、`note`、`second_note`、`data_missing` | `deferred=true` 时只给一条推荐——`check_blend` 是单条检查、不跨条，两组搭配同给会导致剂量叠加没人守 |
| `MealPlanStage` | `direction`、`label`、`open`、`herbs[]`、`amounts`、`title`、`reason` | `open=false` 表示**该方向本次不可用**（方向子集与候选集交集为空，或药材未定）。不是错误，也不得退化成别的方向 |

### 4.4 输出对象

| 对象 | 字段 |
|---|---|
| `Recommendation` | `title`、`herbs[]`、`brew`、`fit_reason`、`cautions[]`、`score` |
| `HerbInBlend` | `name`、`amount_g`(≥0)、`nature`、`flavors[]`、`meridians[]`、`role` |
| `BrewGuide` | `vessel`、`water_ml`(100–2000)、`water_temp_c`(40–100)、`steps[]`、`steep_min`(1–60)、`refill_times`(0–5) |
| `CatalogHerb` | `id`、`name`、`nature`、`flavors[]`、`meridians[]`、`effects[]`、`max_daily_g`、`cautions[]`、`suitable_constitutions[]`、`brewing`、`unsuitable_for[]` |
...
| 饮片必须来自 44 味白名单 | **代码**（候选集 + `check_blend`） | 候选集里挑不到；输出时该味被剔除 |
...
- **44 味 vs 食药物质目录**：`docs/catalog-compliance.md` 记录了白名单与"既是食品又是中药材的物质目录（106 种）"的逐项核对与处置决定。其中一条重要口径：**目录只给合规身份、不含四气**，因此**不得作为体质适配依据**。
...
⚠️ **实测：`combine()` 在 `core/app/` 下只有定义、没有调用点**——多层食材合成**尚未接入主流程**，`resolve_food()` 目前只做单食材判定（`README.md:490-492`、`docs/three-layer-architecture.md:208-210`）。整菜名（如"番茄炒蛋"）要么表里收录，要么整体落到第三层。
...
⚠️ 数据文件的 `_meta.unexecutable_thresholds` 如实记录了两条**当前不可执行**的口径（湿气·高糖原述"每份总糖 ≥ 15 g"、湿气·油腻原述"每 100 g 脂肪 ≥ 20 g"），因为食性表没有糖量与脂肪字段。这是**已知缺口，不是隐藏缺陷**。
...
**✅ 已确认（原列于待确认，现已核实）**：`CONF_SHOW_THRESHOLD` 的引用点只有 `agent2_recommend.py:26/111`、`food_lookup.py:45`、终端 `chat.py:68/106/108`、网页 `index.html:318/513/529` —— **网页壳是通过 `/api/meta` 间接使用，没有硬编码**，与"各壳不要各自硬编码"的设计一致。
...
按此口径，**服务端共 9 条降级出口 = 主链路 3 条 + 兜底函数内部 3 条 + 离线路径 3 条**。

另外必须与这 9 条分开看待的还有四类：**再降一级但仍有推荐**（§7.2 末）、**降级链的尽头（唯一 500 点）**（§7.4）、**刻意不降级的硬失败 5 条**（§7.5）、**看起来像降级但实际不是的 3 类**（§7.6）。

### 7.1 主链路 3 条（`orchestrator.analyze()`）

| # | 触发条件 | 行为与标注 | 锚点 |
|---|---|---|---|
| **1** | **命中高风险关键词** | 不调用模型、不花钱；`recommendations=[]`；`parsed` 只保留 `meal_time` 与一句 summary；`basis.guardrail_applied=["命中高风险关键词：…"]`；`degraded=True` / `degraded_reason="high_risk_group"` / `key_source="not_used"`；`user_message` 引导就医 | 判定 `orchestrator.py:218`；返回 `:224-248`；标注 `:237-238` |
| **2** | **Agent2 抛异常，或返回空推荐** | 转 `matcher.fallback_recommend(parsed, constitution, exclude_herbs, avoid=avoid)`；`degraded=True`；`degraded_reason = str(exc)[:200]`；`rule_hits` 一并带出 | 触发 `orchestrator.py:315-323`（空推荐在 `:322-323` 转成异常）；兜底 `:324-330` |
| **3** | **输出护栏把推荐全拦掉**（且此前未降级过） | 再兜底一次 → **再过一遍护栏** → `degraded=True` / `degraded_reason="guardrail_removed_all"` | `orchestrator.py:338-347` |

> 语义提示：`degraded=True` **不等于故障**——#1 是"刻意不调模型"的正常分支（见 §7.6）。

### 7.2 兜底函数内部的 3 条空出口（`matcher.fallback_recommend()`）

兜底函数的契约是**"永远给出合法且安全的搭配"**（`matcher.py:495-509`）。但有两种情况连它也給不出，于是返回**空推荐 + 专用话术**（仍 200，不是 500）：

| # | 触发条件 | 行为 | 锚点 |
|---|---|---|---|
| **4** | **`exclude_herbs` 正好覆盖整组搭配** | 空推荐 + "你排除的饮片正好是这组搭配的全部，换一组或去掉排除项再试试。" | `matcher.py:518-520` |
| **5** | **体质（含兼夹）硬剔除把整组剔空** | 先退到平和质通用搭配并**照实归因**（是主导体质剔的、兼夹剔的、还是两者都有）；若通用兜底也被剔空 ⇒ 空推荐 + "按你的体质（含兼夹体质）筛下来没有可用饮片，请换个说法或咨询医师。" | 剔除 `matcher.py:523-524`；退通用并归因 `:526-553`（含 `avoid_cleared_blend` / `constitution_cleared_blend` 两个 hits）；空出口 `:537-538` |
| **6** | **搭配里的饮片全不在白名单**（`_make_herbs` 全部跳过） | 空推荐 + "候选饮片不可用，请检查 herbs.json。" | `matcher.py:557-559`；`_make_herbs` 定义 `:208-226` |

**再降一级但仍有推荐（不属上面 3 条空出口）**：`_constitution_default()` 遇到未收录的体质时，**不退化成 `KeyError`**，而是退到 `GENERIC_FALLBACK_KEY="balanced"`，并在理由里写明"该体质的专属搭配尚未收录，已改用平和质的通用搭配"（`matcher.py:247-263`）。它当前收录**全部 9 型**默认搭配（`matcher.py:69-119`），所以这条分支现在是纵深防御。
...
### 7.3 离线路径 3 条（`/api/analyze-offline`）
...
| 字段 | 取值 | 含义 |
|---|---|---|
| `meta.degraded` | `true` / `false` | 是否走了规则兜底。⚠️ **不要单独用它判断故障** |
...
| 项 | 现状 |
|---|---|
| 多层食材合成 | `nature_math.combine()` 已实现并测试，但在 `core/app/` 下**无调用点** ⇒ **未接入主流程**。`resolve_food()` 只判定单一食物；整菜名要么表里收录，要么整体落到第三层 |
...
> 原则：**没核实就不写死**。以下都是我在本次只读复核中未能确证的点，正式引用前请再核一次。
