...
| 7 | **体质模型 5 型 → 9 型**（补阴虚/血瘀/气郁/特禀，对齐 GB/T 46939-2025） | **卡在数据，不在代码。** 代码约 100 行（枚举、`constitution.json` 4 条、提示词 4 行、测试 +6 项）；数据才是大头：`herbs.json` 里 34 味 × `suitable_constitutions` + 27 味 × `unsuitable_for`，针对 4 个新体质共约 **244 个配伍判定**。而 **GB/T 46939-2025 只规定体质分类、问卷、计分与判定阈值，不提供任何饮食或饮片建议** —— 这 244 条无标准可依，必须由懂中医的人判定。防错已就位（见下），所以"只加枚举忘了补数据"现在是**安全的失败**，可以分次推进 |
