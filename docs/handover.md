...
这一节是项目的骨架。**改动代码前先读这八条**，它们解释了绝大多数看似奇怪的实现。
...
本项目回答的是「**吃了之后怎么办**」，不是「**你别吃**」。所有输出都是**补救方向**，不是拦截。

| | 事前提醒（**不做**） | 事后处理（**做**） |
|---|---|---|
| 动作 | 拦、挡、报警 | 不拦不挡，只给补救方向 |
| 话术 | 「你别吃虾」 | 「你吃了虾，那喝点什么」 |
| 例 | 忌口清单、发物拦截、硬禁忌 | 体质 × 极端 → 优先调理方向 |

三条推论（2026-09-20 裁定，见 §6 决策 34）：

- **「发物」不单列**——它是事前概念（忌口），项目把它当普通食物处理。
- **没有「硬禁忌」档**：仲裁顺序只有两级——冲击度 ≥2（第一优先级：护脾胃）→ 体质 × 极端方向（第二优先级）。
- **用户自述过敏走既有高风险分支**：`HIGH_RISK_KEYWORDS` 已含「过敏」，说「我吃虾过敏」今天就会走那条路——那是「用户自述」，不是「发物拦截」。
...
```bash
cd core
python -m pytest tests -q               # 818 项，全离线，约 6 秒
...
### 6.6 项目边界（2026-09-20 新增）

| # | 决策 | 理由 |
|---|---|---|
| 34 | **本项目是「事后处理」工具，不做「事前提醒」**：不拦、不挡、不报警；所有输出都是「吃了之后怎么调」 | 「事前提醒」需要一份**可审定名单**与个体化判据，两者都不存在（发物名单实测：本地语料只有上位类别有据、食物表覆盖约 55%、食物侧无体质通路）。推论：「发物」不单列；仲裁顺序为 冲击度 ≥2 → 体质 × 极端方向，**不设硬禁忌档**；用户自述过敏走既有高风险分支（`HIGH_RISK_KEYWORDS` 含「过敏」）。详见 `docs/pending-items.md` 的 D18／D20／D22 |
...
# 2. 验证一切正常（约 1 分钟，不花钱）
..\core\.venv\Scripts\python.exe -m pytest tests -q          # 期望 818 passed
