...
| 全链路耗时 | 11.3–24.9 s | 12.6–15.6 s | **5.1–5.7 s** |
...
| 11 | **思考强度保持 `low`** | 初期 demo 的取舍（数据表明 `off` 更快更省且不降准确率，见 §7） |
...
| 文件/目录 | 作用 | 是否必需 |
|---|---|---|
| **`core/.venv/`** | **真正装了依赖的虚拟环境（64 个包）**。所有命令都用它 | **是** |
| `.venv/`（仓库根） | ⚠️ **残留的半成品环境（只有 16 个包，缺 pytest/fastapi/httpx/dotenv）**，来自目录改名前的时期 | 否，建议删掉 |
...
| `.idea/` | 编辑器配置 | 否 |
| `NOTES.local.md` | 维护者本地备忘（含本机路径与待办），被 `*.local.md` 忽略 | 否 |
...
| 陷阱 | 现象 | 正解 |
|---|---|---|
| **仓库根有两个 `.venv`，别用错** | 用 `..\.venv\Scripts\python.exe -m pytest` 报 `No module named pytest` | **只用 `core\.venv\Scripts\python.exe`**（它的包数是根 `.venv` 的 4 倍）。根 `.venv` 是目录改名前的残骸 |
