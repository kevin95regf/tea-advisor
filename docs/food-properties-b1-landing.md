...
1. **B1 只覆盖 11 条**。茶饮共 18 条 ⇒ 余 7 条。
   **订正（2026 年 9 月 19 日 B2）**：原文写「余 7 条（普洱/绿茶/红茶/乌龙/大麦茶/水果茶/奶盖茶）
   语料里**确实无据**（50 号文件行 396 把它们与菊花茶并列判 C 档）」——**有两处错**：
   ① 行 396 列的是「**乌龙茶、普洱、菊花茶**」三条，**不含绿茶/红茶**；
   ② 绿茶/红茶**有据**：50 号文件 §4.5 行 390/391 给出「绿茶偏凉（清涤）／红茶偏温」，
      行 395 明写「**绿茶/红茶的一凉一温是教材原话（S1:566），可直接引用**」；
      §5.2 表（行 471）更明确注「⚠️ **绿茶/红茶不在此列**……按 §4.5 的 B 档处理」。
   B2 已于 2026 年 9 月 19 日登记绿茶/红茶（`layer ①`，how=「同条目·品种方向描述」）。
   ⇒ **本项剩余改为 5 条**：普洱／乌龙／大麦茶／水果茶／奶盖茶，按 L3「纯粹无据」留在剩余条里。
