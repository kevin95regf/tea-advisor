...
| | |
|---|---|
| **现状** | **Step 1 + Step 2 均已完成（2026-09-17）**：枚举 / `constitution.json` / 就绪闸门已扩到 9 型；`herbs.json` 写入 10 格「公开来源直接点名」的配伍标注，**九型已全部就绪并对外可见**。**余下的 244 格是完备度问题，不再是功能阻塞**：4 个新体质逐味判定的完整覆盖约为 34 味 × `suitable_constitutions` + 27 味 × `unsuitable_for`，目前只填了 **15** 格（10 + 第二批 5）。<br>同日另有 **cautions 补写批次**：把项目**自有** `cautions` 已明写阴虚不宜的 10 味追加进 `unsuitable_for`（见「附注」），属项目数据一致性修复，不引用外部来源、不含外推。<br>**同日第二批（5 格）**：由外部专业意见裁定 —— 3 格走 `cautions` 软通道（荷叶×阴虚、山楂×阴虚、槐花×血瘀）、1 格硬屏蔽（香薷×阴虚，`evidence`）、1 格升位（山楂×气郁 → `suitable_constitutions`）。见 `docs/herbs-9types-batch2.md` |
...
| **附注** | 已写入的 10 格（公开来源点名）：**宜**——桑椹×阴虚，山楂/红枣/生姜×血瘀，红枣/薄荷×气郁，红枣/山药×特禀；**忌**——乌梅×血瘀、乌梅×气郁。**另有 cautions 批次 10 味**（项目自有数据，属推导非权威依据）：茯苓、陈皮、龙眼肉、生姜、薏苡仁、茉莉花、橘红、藿香、紫苏、佛手 → 阴虚 `unsuitable_for`。<br>**第二批 5 格**（外部专业意见）：香薷×阴虚（**忌**，`evidence` → `unsuitable_for`）、荷叶×阴虚（慎用，`inference` → `cautions`）、山楂×阴虚（慎用，`inference` → `cautions`）、山楂×气郁（宜（倾向），`inference` → `suitable_constitutions`）、槐花×血瘀（慎用（低置信），`inference` → `cautions`）。按 A1 口径，所有写入都属**未人工审核数据**，界面仍标「待验证」 |
...
| **为什么不是顺手能做** | ① 要跨两张表（食物 147 条 + 饮片 34 味），两边的字段结构不同；② 必须处理**置信度过滤** —— 0.3（模型推测）/0.1（判不出）的条目**不该**被反查出来当结果，否则等于把「未验证」当「已验证」用；③ 还要定排序口径（按四气强度排，还是叠加体质适配排）|
