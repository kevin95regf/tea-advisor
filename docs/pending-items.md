...
>
> 快照时间：**2026-09-17**　共 **9** 项挂起（A/B/C/D/E 五类）。
...
| **D2** | 5 个提交未推送 | 项目所有者 | 协作者看不到这批产出 | 待推送 |
...
| | |
|---|---|
| **现状** | 本地 `main` 领先 `origin/main`（锚点 `010db2f`）**若干提交**，全部未推送 |
| **准确数字** | `git log --oneline origin/main..HEAD`（**不在此硬编码**：数字每次本地提交都会变，硬编码必然过期） |
| **影响** | 协作者 nanple 在 GitHub 上看不到这批产出（食药物质目录、九型资料、药典核对、四项决定、交接文档） |
| **决定** | 推送由项目所有者决定，**不属于开发默认动作** |
| **验收标准** | `git log --oneline origin/main..HEAD` 输出为空 |
