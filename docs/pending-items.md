...
>
> 快照时间：**2026-09-18**　共 **13** 项记录在案（A/B/C/D/E 五类），
> 其中 **D2、E1 已解决**、**B3 的阻塞已解除**（九型已上线；244 格的布局中已落地 15 格，余 239 格是完备度而非功能阻塞），其余 10 项待处理。
...
| **E3** | 饮片侧缺「逐味公开依据」链（分叉批次评估的缺口 3） | 开发 | 用户与审核人看不到每味饮片的出处 | 待实施 |
...
| | |
|---|---|
| **现状** | **食物侧有、饮片侧没有。** `docs/food-properties-sources.json` 逐条登记了食性表的来源（原文 + 行号 + 依据等级），而 `herbs.json` 的 34 味**没有任何逐条来源字段** —— 每味的四气五味归经「依据是哪本书的哪一条」，代码与数据里都查不到 |
| **后果** | 用户与审核人只能看到结论（甘、平、归脾经），看不到出处。而这正是本项目**对食物侧已经做到**的事，两边口径不一致 |
| **可借的结构** | 分叉批次里有一套 `EvidenceReference` + `constitution_evidence()` + `Basis.references`，能在终端打印「淄博市卫健委《不同体质适合不同药膳》｜支持：生姜、枸杞子｜URL」，并有护栏「程序会结构化展示来源，你不得虚构其他文献或链接」。**只借结构，不借数据** |
| **需要什么** | ① 建饮片侧来源登记表（结构对齐 `food-properties-sources.json`，27–47 条的规模）；② 把分叉批次的九型矩阵按拍板**只当来源层**逐格走 `rulings` 登记；③ 给 `Basis` 加 `references` 并接到终端与网页 |
| **相关文件** | `docs/food-properties-sources.json`（结构参照）、`docs/agent2-9types-brew-plan.md` §5、`docs/herbs-9types-batch2.md`（`rulings` 登记的口径） |
