...
| **B3** | 244 个配伍判定（九型已上线，余下是完备度） | nanple | 推荐质量（不再阻塞体质模型 5→9 型） | 部分解决 |
...
| | |
|---|---|
| **现状** | **Step 1 + Step 2 均已完成（2026-09-17）**：枚举 / `constitution.json` / 就绪闸门已扩到 9 型；`herbs.json` 写入 10 格「公开来源直接点名」的配伍标注，**九型已全部就绪并对外可见**。**余下的 244 格是完备度问题，不再是功能阻塞**：4 个新体质逐味判定的完整覆盖约为 34 味 × `suitable_constitutions` + 27 味 × `unsuitable_for`，目前只填了 10 格 |
...
| **需要什么** | nanple（或另一位懂中医的人）给出余下的判定；3 处已单列待确认（山楂×阴虚、山楂×气郁、槐花×血瘀） |
...
| **附注** | 已写入的 10 格：**宜**——桑椹×阴虚，山楂/红枣/生姜×血瘀，红枣/薄荷×气郁，红枣/山药×特禀；**忌**——乌梅×血瘀、乌梅×气郁。按 A1 口径，这些属**未人工审核数据**，界面仍标「待验证」 |
